...
sub sendMasterLogin {
	...
	my $mlen = length($msg);
	debug "Sent sendMasterLogin (switch=$sw, len=$mlen, user=$username)\n", "sendPacket", 2;
	# DEBUG: show packet hex for login troubleshooting
	my $hexdump = join(' ', map { sprintf("%02X", ord($_)) } split(//, substr($msg, 0, 20)));
	Log::message("[LOGIN-DEBUG] Packet sent: switch=$sw len=$mlen first20bytes=[$hexdump]\n", "connection");
}
