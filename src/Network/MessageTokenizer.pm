...
sub readNext {
	...
	if ($nextMessageMightBeAccountID) {
		if (length($$buffer) >= 4) {
			...
			# Only set accountID if not already established (e.g., by account_server_info).
			# Some servers send incorrect/garbage bytes when reconnecting to char/map servers.
