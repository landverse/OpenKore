...
sub map_loaded {
	...
	if ($net->version == 1) {
		...
		$messageSender->sendRequestCashItemsList() if (grep { $masterServer->{serverType} eq $_ } qw(bRO idRO_Renewal twRO ROla Landverse)); # tested at bRO 2013.11.30, request for cashitemslist
...
sub login_pin_code_request {
	...
	if ($args->{flag} == 0) { # removed check for seed 0, eA/rA/brA sends a normal seed.
		...
		# Only set accountID if not already set by account_server_info.
		# Some servers (e.g., Landverse) send incorrect/shifted accountID in the PIN packet.
		if (!$accountID) {
			$accountID = $args->{accountID};
			debug sprintf("Account ID (from PIN): %s (%s)\n", unpack('V',$accountID), getHex($accountID));
		} else {
			debug sprintf("Account ID (PIN packet, ignored - already set): %s vs current %s\n", getHex($args->{accountID}), getHex($accountID));
		}
